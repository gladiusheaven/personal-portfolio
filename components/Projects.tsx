...
const ProjectCard: React.FC<{ project: Project }> = ({ project }) => (
  <div className="group relative grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
    ...
    <div className="md:col-span-7 relative z-10">
      <a href={project.liveUrl || project.repoUrl || '#'} target="_blank" rel="noopener noreferrer" className="block rounded-lg overflow-hidden bg-accent">
        <img
    ...
    <div className="md:col-span-5 md:text-right relative z-20">
      ...
      <h3 className="text-2xl font-bold text-lightest-slate hover:text-accent mb-4">
      ...
      <div className="flex justify-end mt-4 space-x-4">
        {project.repoUrl && (
          <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="text-lightest-slate hover:text-accent transition-colors duration-300">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg>
          </a>
        )}
        {project.liveUrl && (
          <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-lightest-slate hover:text-accent transition-colors duration-300">
    ...
    <Section id="projects" title="Things I've Built">
      <div className="relative">
        ...
        <button 
          ...
          className="absolute top-1/2 left-0 md:-left-16 transform -translate-y-1/2 z-20 text-slate hover:text-accent p-2 rounded-full transition-colors duration-300"
        ...
        <button 
          onClick={goToNext} 
          className="absolute top-1/2 right-0 md:-right-16 transform -translate-y-1/2 z-20 text-slate hover:text-accent p-2 rounded-full transition-colors duration-300"
          ...
          {PROJECTS.map((_, slideIndex) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-colors duration-300 ${
                currentIndex === slideIndex ? 'bg-accent' : 'bg-lightest-navy hover:bg-slate'
              }`}
